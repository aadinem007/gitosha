...
| Area | Status | Notes |
|---|---|---|
| CSP | Done | Self + Supabase; `frame-ancestors 'none'`; `object-src 'none'`. Residual: `'unsafe-inline'` / `'unsafe-eval'` for Next — tighten with nonces later |
...
| MFA | N/A (scaffolding) | App has no TOTP UI. Enable MFA in Supabase Auth dashboard when product needs it; operators should use 2FA on Supabase/Xflow/Vercel |
...
| Secrets not in NEXT_PUBLIC_ / client | Done | Service role, Xflow secrets server-only; public = anon + site URL |
...
| Webhook unsigned rejected | Done | Xflow HMAC of `id.timestamp.body` + timestamp skew; 503 if secret missing |
...
| Risk | Mitigation |
|---|---|
| Payment forgery / free entitlement | Server prices only; Xflow signed webhook + intent re-fetch; fulfill only `isFulfillablePlanId` |
...
- CSP for self + Supabase; `frame-ancestors 'none'`; `object-src 'none'`
...
- Timing-safe signature compares (Xflow webhook HMAC)
...
| xflow webhook | 120 / min |

### Money path
- Server plan table only — never trust client amounts
- Xflow-only service layer (`src/lib/payments/`)
- Webhook: official HMAC of `Webhook-Id.Webhook-Timestamp.body` + 5-minute skew + event-id idempotency
- Frontend confirm re-queries Xflow; it cannot mark paid locally
...
4. **Secret rotation** for Xflow / Supabase / Resend
